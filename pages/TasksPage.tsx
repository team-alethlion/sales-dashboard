...
const TasksPage: React.FC = () => {
  ...
  const [viewDate, setViewDate] = useState(new Date(2024, 5, 21)); // Set to June 21, 2024 for demo
  ...
  const KanbanColumn = ({ title, status, color }: { title: string, status: Status, color: string }) => (
          ...
          <div className="space-y-1">
            {dayTasks.slice(0, 3).map(t => (
              <div key={t.id} className={`px-2 py-1 rounded-md text-[8px] font-black uppercase tracking-tight truncate border ${
                ...
                t.priority === 'High' ? 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 border-amber-100' :
  ...
  const renderDayView = () => {
    ...
    return (
      <div className="bg-white dark:bg-slate-900 p-10 min-h-[500px]">
         <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-12">
            <div className="flex items-center gap-6">
              <div className="w-20 h-20 bg-teal-600 text-white rounded-3xl flex flex-col items-center justify-center shadow-xl shadow-teal-900/20">
                 <span className="text-[10px] font-black uppercase tracking-widest">{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][viewDate.getDay()]}</span>
                 <span className="text-3xl font-black">{viewDate.getDate()}</span>
              </div>
              <div>
                 <h3 className="text-2xl font-black text-slate-800 dark:text-slate-100">{viewDate.toLocaleString('default', { month: 'long' })} {viewDate.getFullYear()}</h3>
                 <p className="text-sm text-gray-400 font-bold uppercase tracking-widest mt-1 flex items-center gap-2">
                   <Zap size={14} className="text-teal-600" /> {dayTasks.length} Operational Tasks Scheduled
                 </p>
              </div>
            </div>
            <button 
              onClick={() => handleOpenNewTask(dateStr)}
              className="flex items-center gap-2 px-6 py-3 bg-teal-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-teal-900/10 hover:bg-teal-700 active:scale-95 transition-all"
            >
         ...
         <div className="space-y-6 max-w-3xl">
            {dayTasks.length === 0 ? (
               <div className="py-20 flex flex-col items-center justify-center text-center opacity-30 border-2 border-dashed border-gray-100 dark:border-slate-800 rounded-[2.5rem]">
                  <CalendarIcon size={48} className="text-gray-400 mb-4" />
                  <p className="text-sm font-black uppercase tracking-widest">No activities for today</p>
               </div>
            ) : (
              dayTasks.map(t => (
                <div key={t.id} className="flex items-start gap-6 p-6 bg-gray-50 dark:bg-slate-800/40 rounded-[2rem] border border-gray-100 dark:border-slate-800 hover:border-teal-200 transition-all group">
                   <div className="w-1.5 h-12 bg-teal-600 rounded-full group-hover:scale-y-125 transition-transform" />
                   <div className="flex-1">
                      <div className="flex items-center justify-between">
                         <span className="text-[10px] font-black uppercase text-teal-600 tracking-[0.2em]">{t.category}</span>
                         <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase ${t.priority === 'Urgent' ? 'bg-rose-50 text-rose-600' : 'bg-teal-50 text-teal-600'}`}>{t.priority}</span>
                      </div>
                      <h4 className="text-lg font-black text-slate-800 dark:text-slate-100 mt-2">{t.title}</h4>
                      <p className="text-xs text-gray-400 font-medium mt-1 leading-relaxed">{t.description}</p>
                      <div className="flex items-center gap-4 mt-4 pt-4 border-t border-gray-100 dark:border-slate-700">
                         <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-white dark:bg-slate-800 border flex items-center justify-center text-gray-400"><User size={12}/></div>
                            <span className="text-[10px] font-bold text-gray-400 uppercase">{t.assignee}</span>
                         </div>
                         <div className="flex items-center gap-2">
                            <Clock size={12} className="text-gray-400" />
                            <span className="text-[10px] font-bold text-gray-400 uppercase">9:00 AM Deadline</span>
                         </div>
                      </div>
                   </div>
                   <button className="p-3 bg-white dark:bg-slate-900 rounded-2xl shadow-sm text-gray-300 hover:text-teal-600 transition-colors"><MoreVertical size={20}/></button>
                </div>
              ))
            )}
         </div>
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-500 pb-20">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <input 
              ...
              className="pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-xl text-xs font-medium focus:ring-2 focus:ring-teal-500/30 transition-all w-60 shadow-sm"
            />
          </div>
          <button 
            onClick={() => handleOpenNewTask()}
            className="flex items-center gap-2 px-5 py-2.5 bg-teal-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-xl shadow-teal-900/10 hover:bg-teal-700 active:scale-95 transition-all"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          { label: 'Active Tasks', value: stats.openCount, sub: 'Currently assigned', icon: <Zap size={18} />, color: 'text-teal-600', bg: 'bg-teal-50 dark:bg-teal-900/20' },
          { label: 'Urgent Action', value: stats.urgentCount, sub: 'High risk items', icon: <AlertCircle size={18} />, color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-900/20' },
          { label: 'Successfully Finished', value: stats.completedCount, sub: 'This cycle', icon: <CheckCircle2 size={18} />, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
          { label: 'Missed Deadlines', value: stats.missedCount, sub: 'Requires review', icon: <XCircle size={18} />, color: 'text-gray-400', bg: 'bg-gray-100 dark:bg-slate-800' },
        ].map((stat, i) => (
      ...
      <div className="flex items-center gap-2 p-1 bg-gray-100/50 dark:bg-slate-800/50 rounded-2xl mb-8 w-fit border border-gray-100 dark:border-slate-800 overflow-x-auto no-scrollbar">
        {[
          { id: 'board', name: 'Kanban Board', icon: <Layout size={14} /> },
          { id: 'list', name: 'List Audit', icon: <List size={14} /> },
          { id: 'calendar', name: 'Operational Calendar', icon: <CalendarIcon size={14} /> },
        ].map(tab => (
          <button
            key={tab.id}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50 dark:bg-slate-800/50">
                  ...
                  <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Task Details</th>
                  ...
                  <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Deadline</th>
                ...
                {filteredTasks.map(task => (
                  <tr key={task.id} className="hover:bg-gray-50/30 dark:hover:bg-slate-800/20 transition-colors group">
                    ...
                    <td className="px-8 py-6">
                      ...
                      <p className="text-[10px] text-gray-400 font-medium truncate max-w-[250px]">{task.description}</p>
                    </td>
                    ...
                    <td className="px-8 py-6 text-center font-bold text-[11px] text-gray-400 uppercase tracking-tight">{task.deadline}</td>
                    <td className="px-8 py-6 text-center">
      ...
      {/* Tab Content: CALENDAR VIEW (UPGRADED) */}
      {activeTab === 'calendar' && (
        <div className="animate-in fade-in slide-in-from-left-4 duration-500">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-gray-50 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
            
            {/* Calendar Controls */}
            <div className="p-8 border-b border-gray-50 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gray-50/20 dark:bg-slate-800/20">
               <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 p-1 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-xl shadow-sm">
                    ...
                    <button onClick={handleToday} className="px-4 py-2 text-[10px] font-black uppercase tracking-widest hover:text-teal-600 transition-colors">Today</button>
            ...
            <div className="flex-1 overflow-x-auto">
              {calendarMode === 'month' && (
                <div className="grid grid-cols-7 gap-px bg-gray-100 dark:bg-slate-800 border-b border-gray-100 dark:border-slate-800">
              ...
              {calendarMode === 'week' && (
                <div className="grid grid-cols-7 bg-gray-100 dark:bg-slate-800">
                  {renderWeekView()}
                </div>
              )}
      ...
      {showNewTask && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 py-8">
          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setShowNewTask(false)} />
          <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 animate-in zoom-in-95 duration-300 overflow-hidden flex flex-col max-h-[90vh]">
             <div className="p-8 border-b border-gray-50 dark:border-slate-800 flex justify-between items-center bg-gray-50/20 dark:bg-slate-800/20 shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] mt-1">Operational Directive Protocol</p>
                  </div>
                </div>
                <button onClick={() => setShowNewTask(false)} className="p-2.5 text-gray-300 hover:text-rose-500 transition-all hover:rotate-90">
                  <X size={24} />
                </button>
             </div>
             ...
             <form onSubmit={handleAddTask} className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-black text-gray-400 uppercase ml-1 mb-1 block">Task Title *</label>
                    <input type="text" placeholder="e.g. Weekly Branch Audit" className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-800 rounded-xl border-none focus:ring-2 focus:ring-teal-500/50 text-sm font-bold" required />
                  </div>
                  
                  <div>
                    <label className="text-[10px] font-black text-gray-400 uppercase ml-1 mb-1 block">Description</label>
                    <textarea rows={3} placeholder="Explain the deliverable requirements..." className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-800 rounded-xl border-none focus:ring-2 focus:ring-teal-500/50 text-sm font-medium resize-none" />
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="text-[10px] font-black text-gray-400 uppercase ml-1 mb-1 block">Operational Category</label>
                    ...
                    <div>
                      <label className="text-[10px] font-black text-gray-400 uppercase ml-1 mb-1 block">Priority Tier</label>
                      <select className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-800 rounded-xl border-none focus:ring-2 focus:ring-teal-500/50 text-sm font-black appearance-none cursor-pointer text-teal-600">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="text-[10px] font-black text-gray-400 uppercase ml-1 mb-1 block">Deadline Date</label>
                      <input 
                        type="date" 
                    ...
                    <div>
                      <label className="text-[10px] font-black text-gray-400 uppercase ml-1 mb-1 block">Primary Assignee</label>
                      <div className="relative">
                        <User className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} />
                        <input type="text" placeholder="e.g. Sarah Miller" className="w-full pl-9 pr-4 py-3 bg-gray-50 dark:bg-slate-800 rounded-xl border-none focus:ring-2 focus:ring-teal-500/50 text-sm font-bold" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-teal-50 dark:bg-teal-950/30 rounded-2xl border border-teal-100 dark:border-teal-900/30 flex gap-4">
                   <Timer size={20} className="text-teal-600 shrink-0 mt-0.5" />
                   <p className="text-[10px] font-bold text-teal-700 dark:text-teal-300 leading-relaxed uppercase tracking-wider">
                     The system will generate automated WhatsApp alerts to the assignee 24 hours prior to the deadline to ensure operational continuity.
                   </p>
                </div>
                ...
                <div className="flex gap-4 pt-4 sticky bottom-0 bg-white dark:bg-slate-900 pb-2">
                  ...
                  >
                    {saving ? (
                      ...
                      <>
                        Inject to Workflow <ArrowRight size={14} />
                      </>
                    )}
                  </button>
                </div>
